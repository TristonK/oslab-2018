...
void paint_line(){
   ...
   for(int i=0;i<proc_cnt;i++){
        if(p[i].proc_rate){
            switch (judge_color){
                case 0:
                    printf("\x1b[42m%-20s:%10d%%",p[i].proc_name,p[i].proc_rate);
                    printf("\x1b[0m\n");
                    break;
                case 1:
                    printf("\x1b[43m%-20s:%10d%%",p[i].proc_name,p[i].proc_rate);
                    printf("\x1b[0m\n");
                    break;
                case 2:
                    printf("\x1b[44m%-20s:%10d%%",p[i].proc_name,p[i].proc_rate);
                    printf("\x1b[0m\n");
                    break;
                default:
                    printf("\x1b[46m%-20s:%10d%%",p[i].proc_name,p[i].proc_rate);
                    printf("\x1b[0m\n");
            }
